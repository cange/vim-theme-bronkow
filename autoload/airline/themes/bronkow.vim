ru colors/bronkow/original/colors.vim

let g:airline#themes#bronkow#palette = {}

function! airline#themes#bronkow#refresh()
  """"""""""""""""""""""""""""""""""""""""""""""""
  " Options
  """"""""""""""""""""""""""""""""""""""""""""""""
  let s:background  = get(g:, 'airline_bronkow_bg', &background)
  let s:ansi_colors = get(g:, 'bronkow_termcolors', 16) != 256 && &t_Co >= 16 ? 1 : 0
  let s:tty         = &t_Co == 8

  """"""""""""""""""""""""""""""""""""""""""""""""
  " Colors
  """"""""""""""""""""""""""""""""""""""""""""""""
  " Base colors
  let s:shadeDst  = {'t': s:ansi_colors ?   8 : (s:tty ? '0' : g:colors.shade.darkest.cterm ), 'g': g:colors.shade.darkest.hex  }
  let s:shadeDr   = {'t': s:ansi_colors ? '0' : (s:tty ? '0' : g:colors.shade.darker.cterm  ), 'g': g:colors.shade.darker.hex   }
  let s:shadeD    = {'t': s:ansi_colors ?  10 : (s:tty ? '0' : g:colors.shade.dark.cterm    ), 'g': g:colors.shade.dark.hex     }
  let s:shade     = {'t': s:ansi_colors ?  11 : (s:tty ? '7' : g:colors.shade.default.cterm ), 'g': g:colors.shade.default.hex  }
  let s:shadeL    = {'t': s:ansi_colors ?  12 : (s:tty ? '7' : g:colors.shade.light.cterm   ), 'g': g:colors.shade.light.hex    }
  let s:shadeLr   = {'t': s:ansi_colors ?  14 : (s:tty ? '7' : g:colors.shade.lighter.cterm ), 'g': g:colors.shade.lighter.hex  }
  let s:shadeLst  = {'t': s:ansi_colors ?   7 : (s:tty ? '7' : g:colors.shade.lightest.cterm), 'g': g:colors.shade.lightest.hex }
  let s:redDr     = {'t': s:ansi_colors ?   1 : (s:tty ? '1' : g:colors.red.darker.cterm    ), 'g': g:colors.red.darker.hex     }
  let s:redD      = {'t': s:ansi_colors ?   1 : (s:tty ? '1' : g:colors.red.dark.cterm      ), 'g': g:colors.red.dark.hex       }
  let s:red       = {'t': s:ansi_colors ?   1 : (s:tty ? '1' : g:colors.red.default.cterm   ), 'g': g:colors.red.default.hex    }
  let s:redL      = {'t': s:ansi_colors ?   1 : (s:tty ? '1' : g:colors.red.light.cterm     ), 'g': g:colors.red.light.hex      }
  let s:orangeD   = {'t': s:ansi_colors ?   9 : (s:tty ? '9' : g:colors.orange.dark.cterm   ), 'g': g:colors.orange.dark.hex    }
  let s:orange    = {'t': s:ansi_colors ?   9 : (s:tty ? '9' : g:colors.orange.default.cterm), 'g': g:colors.orange.default.hex }
  let s:orangeL   = {'t': s:ansi_colors ?   9 : (s:tty ? '9' : g:colors.orange.light.cterm  ), 'g': g:colors.orange.light.hex   }
  let s:green     = {'t': s:ansi_colors ?   2 : (s:tty ? '2' : g:colors.green.default.cterm ), 'g': g:colors.green.default.hex  }
  let s:blueD     = {'t': s:ansi_colors ?   4 : (s:tty ? '4' : g:colors.blue.dark.cterm     ), 'g': g:colors.blue.dark.hex      }
  let s:blue      = {'t': s:ansi_colors ?   4 : (s:tty ? '4' : g:colors.blue.default.cterm  ), 'g': g:colors.blue.default.hex   }
  let s:blueL     = {'t': s:ansi_colors ?   4 : (s:tty ? '4' : g:colors.blue.light.cterm    ), 'g': g:colors.blue.light.hex     }
  let s:purpleD   = {'t': s:ansi_colors ?   5 : (s:tty ? '5' : g:colors.purple.dark.cterm   ), 'g': g:colors.purple.dark.hex    }
  let s:purple    = {'t': s:ansi_colors ?   5 : (s:tty ? '5' : g:colors.purple.default.cterm), 'g': g:colors.purple.default.hex }

  let s:white   = {'t': s:ansi_colors ?  15 : (s:tty ? '7' : g:colors.white.default.cterm ), 'g': g:colors.white.default.hex  }
  " let s:yellow  = {'t': s:ansi_colors ?   3 : (s:tty ? '3' : g:colors.orange.light.cterm  ), 'g': g:colors.orange.light.hex   }
  " let s:violet  = {'t': s:ansi_colors ?  13 : (s:tty ? '5' : g:colors.purple.default.cterm), 'g': g:colors.purple.default.hex }
  " let s:cyan    = {'t': s:ansi_colors ?   6 : (s:tty ? '6' : g:colors.blue.light.cterm    ), 'g': g:colors.blue.light.hex     }

  """"""""""""""""""""""""""""""""""""""""""""""""
  " Simple mappings
  " NOTE: These are easily tweakable mappings. The actual mappings get
  " the specific gui and terminal colors from the base color dicts.
  """"""""""""""""""""""""""""""""""""""""""""""""
  " Normal mode
  if s:background == 'dark'
    let s:N1 = [s:shadeLst, s:blue, '']
    let s:N2 = [s:shadeLst, (s:tty ? s:shadeD : s:shade), '']
    let s:N3 = [s:shadeL, s:shadeDr, '']
  else
    let s:N1 = [s:shadeLst, s:shade, 'bold']
    let s:N2 = [(s:tty ? s:shadeD : s:shadeLst), s:shadeLr, '']
    let s:N3 = [s:shadeLr, s:shadeLst, '']
  endif

  let s:NF = [s:orange, s:N3[1], '']
  let s:NW = [s:white, s:orange, '']

  if s:background == 'dark'
    let s:NM =  [s:orangeL, s:N3[1], '']
    let s:NMi = [s:shadeLst, s:N3[1], '']
  else
    let s:NM =  [s:shadeD, s:N3[1], '']
    let s:NMi = [s:shadeDr, s:N3[1], '']
  endif

  " Insert mode
  let s:I1 = [s:shadeD, s:green, '']
  let s:I2 = s:N2
  let s:I3 = s:N3
  let s:IF = s:NF
  let s:IM = s:NM

  " Visual mode
  let s:V1 = [s:white, s:orange, '']
  let s:V2 = s:N2
  let s:V3 = s:N3
  let s:VF = s:NF
  let s:VM = s:NM

  " Replace mode
  let s:R1 = [s:shadeLr, s:redD, '']
  let s:R2 = s:N2
  let s:R3 = s:N3
  let s:RM = s:NM
  let s:RF = s:NF

  " Inactive, according to VertSplit in bronkow
  " (bg dark: shade; bg light: base0)
  if s:background == 'dark'
    let s:IA = [s:shadeDr, s:shade, '']
  else
    let s:IA = [s:shadeLr s:shadeL, '']
  endif

  " let airline#extensions#branch#format

  """"""""""""""""""""""""""""""""""""""""""""""""
  " Actual mappings
  " WARNING: Don't modify this section unless necessary.
  """"""""""""""""""""""""""""""""""""""""""""""""
  let s:NFa = [s:NF[0].g, s:NF[1].g, s:NF[0].t, s:NF[1].t, s:NF[2]]
  let s:IFa = [s:IF[0].g, s:IF[1].g, s:IF[0].t, s:IF[1].t, s:IF[2]]
  let s:VFa = [s:VF[0].g, s:VF[1].g, s:VF[0].t, s:VF[1].t, s:VF[2]]
  let s:RFa = [s:RF[0].g, s:RF[1].g, s:RF[0].t, s:RF[1].t, s:RF[2]]

  let g:airline#themes#bronkow#palette.accents = {
        \ 'red': s:NFa,
        \ }

  let g:airline#themes#bronkow#palette.inactive = airline#themes#generate_color_map(
        \ [s:IA[0].g, s:IA[1].g, s:IA[0].t, s:IA[1].t, s:IA[2]],
        \ [s:IA[0].g, s:IA[1].g, s:IA[0].t, s:IA[1].t, s:IA[2]],
        \ [s:IA[0].g, s:IA[1].g, s:IA[0].t, s:IA[1].t, s:IA[2]])
  let g:airline#themes#bronkow#palette.inactive_modified = {
        \ 'airline_c': [s:NMi[0].g, '', s:NMi[0].t, '', s:NMi[2]]}

  let g:airline#themes#bronkow#palette.normal = airline#themes#generate_color_map(
        \ [s:N1[0].g, s:N1[1].g, s:N1[0].t, s:N1[1].t, s:N1[2]],
        \ [s:N2[0].g, s:N2[1].g, s:N2[0].t, s:N2[1].t, s:N2[2]],
        \ [s:N3[0].g, s:N3[1].g, s:N3[0].t, s:N3[1].t, s:N3[2]])

  let g:airline#themes#bronkow#palette.normal.airline_warning = [
        \ s:shadeDr.g, s:orange.g, s:shadeDr.t, s:orange.t, s:NW[2]]
        " \ s:NW[0].g, s:NW[1].g, s:NW[0].t, s:NW[1].t, s:NW[2]]

  let g:airline#themes#bronkow#palette.normal.airline_error = [
        \ s:shadeDr.g, s:purple.g, s:shadeDr.t, s:purple.t, s:NW[2]]

  let g:airline#themes#bronkow#palette.normal_modified = {
        \ 'airline_c': [s:NM[0].g, s:NM[1].g,
        \ s:NM[0].t, s:NM[1].t, s:NM[2]]}

  let g:airline#themes#bronkow#palette.normal_modified.airline_warning =
    \ g:airline#themes#bronkow#palette.normal.airline_warning

  let g:airline#themes#bronkow#palette.insert = airline#themes#generate_color_map(
        \ [s:I1[0].g, s:I1[1].g, s:I1[0].t, s:I1[1].t, s:I1[2]],
        \ [s:I2[0].g, s:I2[1].g, s:I2[0].t, s:I2[1].t, s:I2[2]],
        \ [s:I3[0].g, s:I3[1].g, s:I3[0].t, s:I3[1].t, s:I3[2]])

  let g:airline#themes#bronkow#palette.insert.airline_warning =
    \ g:airline#themes#bronkow#palette.normal.airline_warning

  let g:airline#themes#bronkow#palette.insert_modified = {
        \ 'airline_c': [s:IM[0].g, s:IM[1].g,
        \ s:IM[0].t, s:IM[1].t, s:IM[2]]}

  let g:airline#themes#bronkow#palette.insert_modified.airline_warning =
    \ g:airline#themes#bronkow#palette.normal.airline_warning

  let g:airline#themes#bronkow#palette.visual = airline#themes#generate_color_map(
        \ [s:V1[0].g, s:V1[1].g, s:V1[0].t, s:V1[1].t, s:V1[2]],
        \ [s:V2[0].g, s:V2[1].g, s:V2[0].t, s:V2[1].t, s:V2[2]],
        \ [s:V3[0].g, s:V3[1].g, s:V3[0].t, s:V3[1].t, s:V3[2]])

  let g:airline#themes#bronkow#palette.visual.airline_warning =
    \ g:airline#themes#bronkow#palette.normal.airline_warning

  let g:airline#themes#bronkow#palette.visual_modified = {
        \ 'airline_c': [s:VM[0].g, s:VM[1].g,
        \ s:VM[0].t, s:VM[1].t, s:VM[2]]}

  let g:airline#themes#bronkow#palette.visual_modified.airline_warning =
    \ g:airline#themes#bronkow#palette.normal.airline_warning

  let g:airline#themes#bronkow#palette.replace = airline#themes#generate_color_map(
        \ [s:R1[0].g, s:R1[1].g, s:R1[0].t, s:R1[1].t, s:R1[2]],
        \ [s:R2[0].g, s:R2[1].g, s:R2[0].t, s:R2[1].t, s:R2[2]],
        \ [s:R3[0].g, s:R3[1].g, s:R3[0].t, s:R3[1].t, s:R3[2]])

  let g:airline#themes#bronkow#palette.replace.airline_warning =
    \ g:airline#themes#bronkow#palette.normal.airline_warning

  let g:airline#themes#bronkow#palette.replace_modified = {
        \ 'airline_c': [s:RM[0].g, s:RM[1].g,
        \ s:RM[0].t, s:RM[1].t, s:RM[2]]}

  let g:airline#themes#bronkow#palette.replace_modified.airline_warning =
    \ g:airline#themes#bronkow#palette.normal.airline_warning

  let g:airline#themes#bronkow#palette.tabline = {}

  let g:airline#themes#bronkow#palette.tabline.airline_tab = [
        \ s:I2[0].g, s:I2[1].g, s:I2[0].t, s:I2[1].t, s:I2[2]]

  let g:airline#themes#bronkow#palette.tabline.airline_tabtype = [
        \ s:N2[0].g, s:N2[1].g, s:N2[0].t, s:N2[1].t, s:N2[2]]

  " CtrlP plugin
  let g:airline#themes#bronkow#palette.ctrlp = airline#extensions#ctrlp#generate_color_map(
        \ [ s:shadeL.g, s:shadeD.g,  s:shadeL.t, s:shadeD.t,  ''     ],
        \ [ s:shade.g,  s:shadeDr.g, s:shade.t,  s:shadeDr.t, 'bold' ],
        \ [ s:blueL.g,  s:blue.g,    s:blueL.t,  s:blue.t,    'bold' ])

endfunction

function! airline#themes#bronkow#order()
   let g:airline_section_a = airline#section#create(['branch'])
   let g:airline_section_b = airline#section#create([])
   let g:airline_section_x = airline#section#create(['ffenc'])
   let g:airline_section_y = airline#section#create(['filetype'])
endfunction

call airline#themes#bronkow#refresh()
call airline#themes#bronkow#order()
